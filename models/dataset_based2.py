...
class img_selector(nn.Module):
    def __init__(self,net,device,stage,input_size=224,num_crop=10,factor=2,ipc=60):
        super().__init__()
        self.net=net
        self.n=20
        self.stage=stage
...
def save_images(save_path, images_fake,images_real, class_id):
    ...
    for id in range(images_fake.shape[0]):
        ...
        place_to_store = dir_path + "class{:05d}_id{:05d}.png".format(class_id, id)
...
def batched_forward(model, tensor,targets, batch_size,device):
    ...
    with torch.no_grad():
        for i in range(0, total_samples, batch_size):
            ...
            output = model(data_dict,inference=True)['cls']
...
def selector(n, model, images, labels, size,device, m=10):
    with torch.no_grad():
        ...
        preds = batched_forward(model, pad(images, size).to(device),labels, batch_size,device)
        # [mipc * m]
