...
def build_net():
    # 创建 Swin Transformer V2 模型，不加载默认的预训练权重
    model = create_model('swinv2_base_window8_256.ms_in1k', pretrained=False, num_classes=2)

    # 定义预训练权重的路径
    weight_path = '/home/tangshuai/dmp-bk/pre_weights/pytorch_model.bin'
    ...
    # 替换分类头的权重和偏置
    fc = nn.Linear(model.head.in_features, 2)
    pretrained_dict["head.fc.weight"] = fc.weight.data
    pretrained_dict["head.fc.bias"] = fc.bias.data

    # 过滤掉不匹配的键（如 'attn_mask'）
...
class swinT(BaseLearner):
    ...
    def _train(self, train_loader, val_loader):
        self._network.to(self._device)
        e_optimizer = optim.Adam(self._network.head.parameters(), lr=lrate, weight_decay=weight_decay)
