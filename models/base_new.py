...
class BaseLearner(object):
    ...
    def eval_task(self, type='test'):
        if type == 'out':
            ...
            loader=self.train_loader_no_shuffle
            embeddings=self._eval_cnn(type, loader)
            return embeddings
        else:
    ...
    def _eval_cnn(self, type, loader):
        ...
        for i, (_, inputs, targets, orders) in enumerate(loader):
            inputs = inputs.to(self._device)
            if type == 'out' or self.args['run_type'] == 'train' or self.args['run_type'] == 'train_bak_dfil' or type=='replay':
                with torch.no_grad():
                    if type=='replay':
                        outputs = self._network(inputs)["logits_pro"]
                        # print(outputs.size())
                        # print('1')
                    else:
