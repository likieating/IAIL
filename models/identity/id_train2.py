...
def compute_acc(device,model,loader,_cur_task):
    model.eval()
    correct, total = 0, 0
    for i, (_, inputs, targets, order) in enumerate(loader):
        ...
        label_spe = torch.where(targets == 0, 0, _cur_task + 1)
        data_dict = {
            'image': inputs,
            'label': targets,
            'label_spe': label_spe,
    ...
    def _init_train(self, train_loader, val_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets, order) in enumerate(train_loader):
                ...
                inputs=torch.cat([inputs[:,0],inputs[:,1]],dim=0)
                targets=torch.cat([targets[:,0],targets[:,1]],dim=0)
                label_spe=torch.where(targets==0,0,self._cur_task+1)
                data_dict={
                    'image':inputs,
                    'label':targets,
                    'label_spe':label_spe,
            ...
            print(train_acc)
            val_acc = compute_acc(self._device,self._network, val_loader,self._cur_task)
    ...
    def _update_representation(self, train_loader, val_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets, order) in enumerate(self.train_loader):
                ...
                loss_all = self._network.get_losses(data_dict, pred_dict)
                loss = loss_all['common']+loss_all['specific']*spe+loss_all['contrastive']*contra+kd*kd_loss+fd*fd_loss+loss_all['reconstruction']*recon
