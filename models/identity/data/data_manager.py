...
class DataManager(object):
   ...
   #得到增量学习的序列
    ...
    def get_dataset(
        ...
        if mode == "train":
            trsf = transforms.Compose([*self._train_trsf, *self._common_trsf,*self.albu_trsf])
        elif mode == "flip":
...
#进行一些数据在进入模型训练之前的通用预处理：必要的变换
    def _setup_data(self, dataset_name, train_data_path, val_data_path, test_data_path, out_data_path, shuffle, seed, debug):
        ...
        self.albu_trsf=idata.albumentations_transform
...
class DummyDataset(Dataset):
    def __init__(self, dataset_name, images, orders, resize_size, trsf, mode, use_path=False, balance=True):
