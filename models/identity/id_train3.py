...
import logging
import numpy as np
from models.identity.networks.ucf_detector_3_ts import UCFDetector_
...
class identity3(BaseLearner):
    def __init__(self, args,window_size=3):
        super().__init__(args)
        self.args = args
        self._old_network = None
        self._network = UCFDetector_()
        self.best_model_path = ["/home/tangshuai/dmp-bk/useful/0126-20-04-55-782_OSMA_split1_pretrained_vit_b16_224_in21k_adapter_1993_B0_Inc1/task0/val_acc96.22_model.pth"]
        # self.best_model_path = [
        #    "/home/tangshuai/dmp-bk/logs/benchmark/OSMA/split1/identity2/0319-14-00-37-345_OSMA_split1_xception_1993_B0_Inc1/task0/val_acc95.9_model.pth"]
    ...
    def _init_train(self, train_loader, val_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets, order) in enumerate(train_loader):
                ...
                loss = loss_all['common'] + loss_all['contrastive']*contra + loss_all['reconstruction']*recon
            ...
            info = "Task {}, Epoch {}/{} => loss {:.4f}, train_accy {:.2f}, val_accy {:.2f}, lr {},contra {}, recon {}".format(
                self._cur_task,
                epoch + 1,
                epochs,
    ...
    def _update_representation(self, train_loader, val_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets, order) in enumerate(self.train_loader):
                ...
                kd_loss = _KD_loss(pred_dict['cls'], pred_dict_old['cls'], T=2)
                # fd_loss = _FD_loss(pred_dict['feat'], pred_dict_old['feat'])
                loss_all = self._network.get_losses(data_dict, pred_dict)
                loss = loss_all['common'] + loss_all['contrastive'] * contra + kd * kd_loss
            ...
            info = "Task {}, Epoch {}/{} => loss_sha {:.4f}, loss_con {:.4f}, loss_kd {:.4f}, train_accy {:.2f}, val_accy {:.2f}, lr {}".format(
                self._cur_task,
                epoch + 1,
                epochs,
