...
class djy_try2(BaseLearner):
    def __init__(self, args,window_size=3):
        ...
        self._network = MAT(device=self._device).to(self._device)
        self.best_model_path = ["/home/tangshuai/dmp-bk/logs/benchmark/OSMA/split1/djy_try2/0124-20-41-18-546_OSMA_split1_pretrained_vit_b16_224_in21k_adapter_1993_B0_Inc1/task0/val_acc94.55_model.pth"]
        self._old_adapter=[]
        self.data_selector=img_selector(self._network,self._device,self._cur_task+1)
        self.img_app_real=torch.zeros(0,3,224)
        self.img_app_fake=torch.zeros(0,3,224)
        self.AG = AGDA()
    ...
    def _init_train(self, train_loader, val_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets, order) in enumerate(train_loader):
                ...
                loss,logits,features = self._network(inputs,targets, train_batch=True, AG=self.AG)
                # loss = cls_loss
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
